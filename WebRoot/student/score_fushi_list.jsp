...
<span style="margin-left:50%; color:red;">复试成绩</span>
</div>
</div>
		 ...
		 <thead>
			<tr>
			  ...
			  <!-- <th class="hidden-480">操作</th> -->
			    ...
			    <li <c:if test="${page.currPage eq '1'}"> class="disabled"</c:if>><a <c:if test="${page.currPage ne '1'}">href="ScoreServlet?action=findAll_fushi&sname=${sname}&currPage=${page.currPage -1}"</c:if>>上一页</a></li>
			    <c:if test="${page.totalPage eq '0'}"><li> class="active"<a href="ScoreServlet?action=findAll_fushi&currPage=1&sname=${sname}">1</a></li></c:if>
			    <c:forEach begin="1" end="${page.totalPage }" var="var">
			    	<li <c:if test="${var eq page.currPage}"> class="active"</c:if>><a href="ScoreServlet?action=findAll_fushi&currPage=${var}&sname=${sname}">${var}</a></li>
			    </c:forEach>
			    <li  <c:if test="${page.currPage eq page.totalPage }"> class="disabled"</c:if>><a <c:if test="${page.currPage ne page.totalPage }">href="ScoreServlet?action=findAll_fushi&sname=${sname}&currPage=${page.currPage +1}"</c:if>>下一页</a></li>
			  </ul>	
         		</td>
			</tr>
				</tbody>
