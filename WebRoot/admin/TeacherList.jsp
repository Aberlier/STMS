...
<a href="AdminServlet?action=addtea_before"  class="btn button_btn bg-deep-blue" title="添加教师"><i class="fa  fa-edit"></i>&nbsp;添加教师</a>
...
<div class="compete_list" >
       ...
       <table id="sample_table" class="table table_list table_striped table-bordered dataTable no-footer">
		 <thead>
		    ...
		    <ul class="pagination">
			    <li <c:if test="${page.currPage eq '1'}"> class="disabled"</c:if>><a <c:if test="${page.currPage ne '1'}">href="AdminServlet?action=findAll_tea&currPage=${page.currPage -1}"</c:if>>上一页</a></li>
			    <c:if test="${page.totalPage eq '0'}"><li> <!-- class="active" --><a href="AdminServlet?action=findAll_tea&currPage=1">1</a></li></c:if>
			    <c:forEach begin="1" end="${page.totalPage }" var="var">
			    	<li <c:if test="${var eq page.currPage}"> class="active"</c:if>><a href="AdminServlet?action=findAll_tea&currPage=${var}">${var}</a></li>
			    </c:forEach>
			    <li  <c:if test="${page.currPage eq page.totalPage }"> class="disabled"</c:if>><a <c:if test="${page.currPage ne page.totalPage }">href="AdminServlet?action=findAll_tea&currPage=${page.currPage +1}"</c:if>>下一页</a></li>
			  </ul>
