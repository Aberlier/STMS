...
<a href="StudentServlet?action=addbaokao_before&headmaster=${headmaster}"  class="btn button_btn bg-deep-blue" title="添加报考"><i class="fa  fa-edit"></i>&nbsp;添加报考</a>
		 ...
		 <thead>
            ...
            <c:forEach items="${list}" var="list">
			  <tr>
                ...
                <a title="删除" href="StudentServlet?action=del_baokao&id=${list.id}"  class="btn button_btn btn-danger">删除</a>
                </td>
			   </tr>
			  </c:forEach>
			  ...
			  <tr >
				<td align="center" colspan="17" bgcolor="#ffffff" >	
		   	 	<ul class="pagination">
			    <li <c:if test="${page.currPage eq '1'}"> class="disabled"</c:if>><a <c:if test="${page.currPage ne '1'}">href="StudentServlet?action=findAll_baokao&headmaster=${headmaster}&role=${role}&currPage=${page.currPage -1}"</c:if>>上一页</a></li>
			    <c:if test="${page.totalPage eq '0'}"><li><a href="StudentServlet?action=findAll_baokao&currPage=1&headmaster=${headmaster}&role=${role}">1</a></li></c:if>
			    <c:forEach begin="1" end="${page.totalPage }" var="var">
			    <li <c:if test="${var eq page.currPage}"> class="active"</c:if>><a href="StudentServlet?action=findAll_baokao&headmaster=${headmaster}&role=${role}&currPage=${var}">${var}</a></li>
			    </c:forEach>
			    <li  <c:if test="${page.currPage eq page.totalPage }"> class="disabled"</c:if>><a <c:if test="${page.currPage ne page.totalPage }">href="StudentServlet?action=findAll_baokao&headmaster=${headmaster}&role=${role}&currPage=${page.currPage +1}"</c:if>>下一页</a></li>
			  </ul>	
         		</td>
			</tr>
		      </tbody>
