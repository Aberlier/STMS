...
<a href="StudentServlet?action=addstu_before&headmaster=${headmaster}"  class="btn button_btn bg-deep-blue" title="添加学生"><i class="fa  fa-edit"></i>&nbsp;添加学生</a>
 
 
  
   <div class="search  clearfix">
  <form action="StudentServlet?action=findbyId&headmaster=${headmaster}" name="form2" id="form2" method="post">
       ...
       <table id="sample_table" class="table table_list table_striped table-bordered dataTable no-footer">
		 <thead>
            ...
            <c:forEach items="${list}" var="list">
			  ...
			  <tr>
				...
				<td>${list.parentstel}</td>
                 <td>${list.headmaster}</td>
				<td><a title="编辑" onclick="myselfinfo(${list.id})" href="javascript:;" class="btn button_btn bg-deep-blue">编辑</a>        
                 <a title="删除" href="StudentServlet?action=deletestu&sname=${list.sname}&headmaster=${list.headmaster}" class="btn button_btn btn-danger">删除</a>
                 <a class="btn button_btn btn-danger" href="<%=basePath%>teacher/stu_chart.jsp?sname=${list.sname}">成绩分布</a>
                 </td>
			...
			<tr >
			    ...
			    <li <c:if test="${page.currPage eq '1'}"> class="disabled"</c:if>><a <c:if test="${page.currPage ne '1'}">href="StudentServlet?action=findAll&headmaster=${headmaster}&currPage=${page.currPage -1}"</c:if>>上一页</a></li>
			    <c:if test="${page.totalPage eq '0'}"><li><a href="StudentServlet?action=findAll&headmaster=${headmaster}&currPage=1">1</a></li></c:if>
			    <c:forEach begin="1" end="${page.totalPage }" var="var">
			    	<li <c:if test="${var eq page.currPage}"> class="active"</c:if>><a href="StudentServlet?action=findAll&headmaster=${headmaster}&currPage=${var}">${var}</a></li>
			    </c:forEach>
			    <li  <c:if test="${page.currPage eq page.totalPage }"> class="disabled"</c:if>><a <c:if test="${page.currPage ne page.totalPage }">href="StudentServlet?action=findAll&headmaster=${headmaster}&currPage=${page.currPage +1}"</c:if>>下一页</a></li>
			  </ul>	
         		</td>
			</tr>
		      </tbody>
