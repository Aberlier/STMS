...
<div class="compete_list" style="width: 100%;overflow-x:scroll;">
       ...
       <table id="sample_table" class="table table_list table_striped table-bordered dataTable no-footer"style="width:160%">
		 <thead>
			<tr>
			...
			<tr >
		   	 ...
		   	 <ul class="pagination">
		   	<%--  <c:if test="${admin_query_org!=null}">
			    ...
			    <li <c:if test="${page.currPage eq '1'}"> class="disabled"</c:if>><a <c:if test="${page.currPage ne '1'}">href="StudentServlet?action=findbyId&sname=${sname}&headmaster=${headmaster}&currPage=${page.currPage -1}"</c:if>>上一页</a></li>
			    <c:if test="${page.totalPage eq '0'}"><li><a href="StudentServlet?action=findbyId&sname=${sname}&headmaster=${headmaster}&currPage=1">1</a></li></c:if>
			    <c:forEach begin="1" end="${page.totalPage }" var="var">
			    	<li <c:if test="${var eq page.currPage}"> class="active"</c:if>><a href="StudentServlet?action=findbyId&sname=${sname}&headmaster=${headmaster}&currPage=${var}">${var}</a></li>
			    </c:forEach>
			    <li  <c:if test="${page.currPage eq page.totalPage }"> class="disabled"</c:if>><a <c:if test="${page.currPage ne page.totalPage }">href="StudentServlet?action=findbyId&sname=${sname}&headmaster=${headmaster}&currPage=${page.currPage +1}"</c:if>>下一页</a></li>
