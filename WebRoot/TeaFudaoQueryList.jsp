		 ...
		 <thead>
            ...
            <c:forEach items="${list}" var="list">
			 ...
			 <tr >
		   	 ...
		   	 <li <c:if test="${page.currPage eq '1'}"> class="disabled"</c:if>><a <c:if test="${page.currPage ne '1'}">href="AdminServlet?action=teaquery&admin_query_org=${admin_query_org}&currPage=${page.currPage -1}"</c:if>>上一页</a></li>
			    <c:if test="${page.totalPage eq '0'}"><li><a href="AdminServlet?action=teaquery&admin_query_org=${admin_query_org}&currPage=1">1</a></li></c:if>
			    <c:forEach begin="1" end="${page.totalPage }" var="var">
			    	<li <c:if test="${var eq page.currPage}"> class="active"</c:if>><a href="AdminServlet?action=teaquery&admin_query_org=${admin_query_org}&currPage=${var}">${var}</a></li>
			    </c:forEach>
			    <li  <c:if test="${page.currPage eq page.totalPage }"> class="disabled"</c:if>><a <c:if test="${page.currPage ne page.totalPage }">href="AdminServlet?action=teaquery&admin_query_org=${admin_query_org}&currPage=${page.currPage +1}"</c:if>>下一页</a></li>
